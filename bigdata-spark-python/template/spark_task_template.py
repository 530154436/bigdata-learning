...
from src.offline.factory.create_spark_session import CreateSparkSession
from src.offline.template.run_batch_param import SparkRunBachParam
from src.offline.template.alarms import DingDingAlarm
from src.offline.template.group_strategy import DataGroupDefaultStrategy
from src.offline.utils import hive_ddl_util
from src.offline.utils.time_util import DateUtil
...
class SparkTaskTemplate:
    ...
    def pipeline(self, rdd: RDD, group_id: int = 0) -> List[dict]:
        ...
        df: DataFrame = rdd \
            ...
            .toDF(schema=self.param.dst_tlb_cols) \
            .persist(StorageLevel.MEMORY_AND_DISK)
        # df.show()
        # df.printSchema
        ...
        df.coalesce(num_partitions)\
            ...
            .mode("append") \
            .partitionBy("dt") \
            .saveAsTable(self.param.dst_tbl_name)
    ...
    def init_dst_tbl(self, spark_session: SparkSession):
        ...
        sql = hive_ddl_util.generate_create_tbl_sql(self.param.dst_tlb_cols,
                                                    table_name=self.param.dst_tbl_name,
    ...
    def run(self):
        ...
        spark_session = CreateSparkSession(self.task_name, **conf) \
            .get_spark_session()
