...
class SparkRunBachParam(object):
    ...
    def __init__(self, **kwargs):
        ...
        :param from_tbl_name: 取数表,待处理的数据集合
          ...
          -dst_tbl_name: 保存结果表-${from_tbl_name}_log,作为为记录表,记录每次任务的所有信息(结果、异常信息等)
        ...
        # log结果记录表
        self.dst_tbl_name: str = self.from_tbl_name + "_log"
        if kwargs.get("dst_tbl_name"):
            self.dst_tbl_name = kwargs.get("dst_tbl_name")
        self.dst_tlb_cols: Union[List[str], StructType] = StructType([
    ...
    @staticmethod
    def _parse_kwargs(string: str, parse_int: int = True) -> dict:
        others = dict()
    ...
    def parse_args(cls) -> "SparkRunBachParam":
        ...
        parser.add_argument('--from_tbl_name', type=str, required=True, help='取数表')
